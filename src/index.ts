...
const renderElement = async (element: HTMLElement, opts: Partial<Options>): Promise<HTMLCanvasElement> => {
    ...
    // 处理video
    //const videoRoot = parseTreeVideo(clonedElement,canvas);

    console.log(canvas);

    console.log(element)

    $(element).find("video").each(function(index:any,ele:Element){

        //console.log(parseBounds(ele);

        let oBoxBound: Bounds = parseBounds(element);
        let oVideoBound: Bounds = parseBounds(ele);

        console.log(oBoxBound)

        console.log(oVideoBound);

        let startCanvasX: number = oVideoBound.left;
        let startCanvasY: number = oVideoBound.top;
        ...
        context.drawImage($("#" + $(ele).attr("id")).get(0), 0, 0, oVideoBound.width, oVideoBound.height,startCanvasX,startCanvasY,oVideoBound.width, oVideoBound.height);//绘制视频
